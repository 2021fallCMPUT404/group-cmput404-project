    ...
    def get(self, request, AUTHOR_ID, POST_ID, format=None):
        try:
            ...
            if AUTHOR_ID != post.author.id:
                return JsonResponse(
                {'message': 'The provided author in url does have the request post'})
    ...
    def get(self, request, AUTHOR_ID, POST_ID, COMMENT_ID, format=None):
        try:
            ...
            if comment.post.id != post.id:
                return JsonResponse(
                {'message': 'The requested post does not have the requested comment'},
                status=status.HTTP_404_NOT_FOUND)
            if post.author != AUTHOR_ID:
                return JsonResponse(
                {'message': 'The requested author does not have the requested post'},
                status=status.HTTP_404_NOT_FOUND)
